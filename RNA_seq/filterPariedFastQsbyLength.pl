...
my $fastq2;
my $len;
my $Results=GetOptions("1=s"=>\$fastq1, "2=s"=>\$fastq2, "len=s" =>\$len);
my $output_file1 = $fastq1.".filtered.txt";
my $output_file2 = $fastq2.".filtered.txt";
...
while (my $line1=<FASTQ_1>) {
	...
	if ($i ==1 && $line1 =~ m/^\@/) {
		...
		chomp $line1;
		chomp $line2;
		if (length($reads1) >= $len && length($reads2) >= $len) {
