...
if ($CNV_file ne "" && $output_new_vcf ne "") { 
	...
	while (my $line=<CNV>) {
		...
		if (ElementInArray(\@sample_names, $id) eq "YES") { # if the ID is in sample names
			...
			my $temp_string="$chr\t$start\t.\t$ref\t$alt\t31\tPASS\tIMPRECISE;SVTYPE=$svtype;END=$end;SVLEN=$svlen;ANNO_CNV_BF=$elements[9];ANNO_CNV_EXP_READS=$elements[10];ANNO_CNV_OBS_READS=$elements[11];ANNO_CNV_RATIO=$elements[12];ANNO_TRUE_START=$elements[5]\tGT:GQ\t./.:31";
...
##INFO=<ID=ANNO_TRUE_START,Number=1,Type=Float,Description="Original start position of the CNV, could be different from the vcf line">
