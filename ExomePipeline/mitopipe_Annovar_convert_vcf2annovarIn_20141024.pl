...
line: foreach $Line (<INPUT>){
	...
	if ($linesplit[0] !~ m/M|m/) {next;} #only process Mitochondrial genome.
	if ($linesplit[0] =~ m/^m$/) {$linesplit[0] = "MT";}
	...
	for(my $i=0;$i<scalar(@sample_call_split);$i++){
		...
		if ($sample !~ m/\.\/\./) {
			...
			if( $first_V eq $variants) {
				...
				my %HT_AD;
				for(my $i=0; $i < scalar @HT_array; $i ++) {
					if ($HT_array[$i] == 0) {
						$HT_AD{0} = $AD_array[$i]; 
					} else {
						$HT_AD{$V[$HT_array[$i]-1]} = $AD_array[$i];
					}
				}
				############################################# here
				my $V_index;
