...
print "# 1. Major change: will filter variants based on cg69 & the two inHouse MAFs (current thresthold 0.05)#\n";
print "# 2. Move 7 columns to the back                                                                       #\n";
print "# Including: VariantQual, GeneID, SNPorINDEL, genomicSuperdump, cg69, inhouseMAF, inhouseGATK_MAF     #\n";
print "# 3. Added 'Everything' sheet to including variants without any filter applied                        #\n";
print "# 4. Refinded compound het sheet.                                                                     #\n";
print "# 5. GATK MAFs is queried with Annovar format. inHouse_MAFs will follow the same in the next version. #\n";
print "#######################################################################################################\n";
...
'exomiserXLS=s' => \$exomiserXLSfiles, 'out=s' => \$output_excel) || defined $help );
...
while (my $line=<INPUT>) {
	if ($line !~ m/^Chr\t/) {
		...
		.$elements[5]."\t".$elements[7]."\t".$elements[8]."\t".$elements[9]
		."\t".$elements[11]."\t".$elements[12]."\t".$elements[13]."\t".$elements[15]."\t".$elements[16]."\t".$elements[$length-11]."\t"
		.$elements[$length-9]."\t".$elements[19]."\t".$elements[20]."\t".
		$elements[22]."\t".$elements[25].":".$elements[27].":".$elements[29].":".$elements[31]."\t".$elements[46]."\t";
		
		#re-arrange columns
		
		my $string_2 = $elements[43]."\t".$elements[$length-10]."\t".$elements[18]."\t".$elements[$length-12]."\t".$elements[21]
		."\t".$elements[$length-8]."\t".$elements[$length-7]."\t".
		$elements[44]."\t".$elements[45]."\t".$elements[$length-6]."\t".$elements[$length-5]."\t".$elements[$length-4]
		."\t".$elements[$length-3]."\t".$elements[$length-2]."\t".$elements[$length-1]."\t".$elements[$length]."\t".$elements[17]."\t".$elements[23]
		."\t".$elements[24]."\t".$elements[25]."\t".$elements[26]."\t".$elements[27]."\t".$elements[28]."\t".$elements[29]."\t".$elements[30]."\t".
		$elements[31]."\t".$elements[32]."\t".$elements[33]."\t".$elements[34]."\t".$elements[35]."\t".$elements[36]."\t".$elements[37]; #re-arrange columns
		
		if($rr_na_sum < scalar @sample_names) { # RR and NA are removed
			push @output_everything, $string_1.$sample_string.$string_2;
		}
		
		if($elements[7] eq "synonymous SNV" && $elements[11] eq "synonymous SNV" && $elements[15] eq "synonymous SNV"  ) { # eliminate synonymous SNV
			next;
		}
		if($elements[5] !~ m/(^exonic|^splicing|^UTR)/i &&  $elements[9] !~ m/(^exonic|^splicing|^UTR)/i && $elements[13] !~ m/(^exonic|^splicing|^UTR)/i  ) { # eliminate intronic or intergenic variants
			next;
		}
...
my @header = 
('Chr','Start','Ref','Obs','Func.ensGene','ExonicFunc.ensGene','AAChange.ensGene','Func.knownGene','ExonicFunc.knownGene',
'AAChange.knownGene','Func.refGene','ExonicFunc.refGene','AAChange.refGene','GeneName','isInterested',
'ESP6500_ALL','1000g2012feb_ALL','dbSNP137', 'Predictions','Format');
...
my @header_2 = ('VariantCall_quality', 'GeneID', 'genomicSuperDups', 'SNPorINDEL', 'cg69', 'InHouseMAF', 'InHouseMAF_GATK'
, 'Filter', 'INFO', 'GoTerm', 'WikiGene_Description', 'MIM_Gene_Description', 'OMIM_Gene_Description', 
'GeneCard Link', 'OMIM Link', 'Uniprot Link','phastConsElements46way','LJB2_SIFT','LJB2_PolyPhen2_HDIV','LJB2_PP2_HDIV_Pred','LJB2_PolyPhen2_HVAR',
