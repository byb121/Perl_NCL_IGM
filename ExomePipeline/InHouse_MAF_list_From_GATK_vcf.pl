...
my $Results=GetOptions("vcfDir=s"=>\$vcfDir, "DupSamples=s"=>\$DupSamples);

#Read in vcf files
my @files;
$vcfDir =~  s/\/$//;
opendir DIR, $vcfDir or die "cannot open dir $vcfDir\n";
	...
	line: foreach my $Line (<INPUT>){
		...
		foreach my $v (@vars){
			...
			if ($ref_now =~ m/^[atgcATGC]$/) { # singlge base ref
				#insertion
				if(length($v_now)>length($ref_now)){$ref_now="-";$v_now=~s/^.//;$pos_1_now++;$pos_2_now=$pos_1_now;} #remove first 'ref' base
...
my $output_file = $vcfDir."/"."InHouse_OnTarget_GATK_MAFs.txt_".$exomecount."exomes";
