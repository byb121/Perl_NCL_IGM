...
# notes for migration of this whole pipe to other platform than FMS cluster:
# check scripts for using of scrach spaces

my $help;
my $sample_table;
my $somaticPi_dir="/sharedlustre/IGM/TY_group/SomaticPi_20151110";
my $ref="b37"; #can only be b37 or hg38
my $SEQ_PLATFORM="Illumina";
my $Library_ID_prefix="YX.somaticPi";

if ( @ARGV < 1 || ! GetOptions('help|?' => \$help, "in=s"=>\$sample_table, "somaticPi=s"=>\$somaticPi_dir, 
	'platform=s' => \$SEQ_PLATFORM, 'LibraryID=s' => \$Library_ID_prefix) 
	|| defined $help ) {
	usage(); exit;
}
$somaticPi_dir =~ s/\/$//;
...
#determine the ref directory
if($ref == "b37") {
	
} elsif ($ref == "hg38") {
	
} else {
	print "ref is not supported, check the ref string or make changes within this perl script.\n";
	print "The currently required ref is $ref\n";
	exit;
}
...
my $QC_script = $somaticPi_dir."/modules/QC_redup.sh"; if (! -e $QC_script) {print "Error: Can not locate script: $QC_script\n"; exit;}
my $align_script = $somaticPi_dir."/modules/map_call_filter_annotate.sh"; if (! -e $align_script) {print "Error: Can not locate script: $align_script\n"; exit;}
my $bed = $somaticPi_dir."/modules/mito.bed"; if (! -e $bed) {print "Error: Can not locate bed file: $bed\n"; exit;}
my $detectLanes_script = $somaticPi_dir."/modules/detectSampleLanes.pl"; if (! -e $detectLanes_script) {print "Error: Can not locate script: $detectLanes_script\n"; exit;}
...
foreach my $sample (keys %in_paras) {
	my $SAMPLE_ID=$sample;
	my $JOB_ID="somaticPi_QC_".$Library_ID_prefix."_".$sample;
	...
	foreach my $in_paras_set (@temp) {
		...
		my $LANE = $temp_sets[2];
		#print "qsub -N $JOB_ID $QC_script $SAMPLE_ID $OUTPUT_DIR $somaticPi_dir $FASTQ1 $FASTQ2 $LANE\n";
		`qsub -N $JOB_ID $QC_script $SAMPLE_ID $OUTPUT_DIR $somaticPi_dir $FASTQ1 $FASTQ2 $LANE`;
	}
	my $JOB_ID_2="somaticPi_Mapping_".$Library_ID_prefix."_".$sample;
	#print "qsub -N $JOB_ID_2 -hold_jid $JOB_ID $align_script $SAMPLE_ID $OUTPUT_DIR $somaticPi_dir $SEQ_PLATFORM $Library_ID_prefix\n";
	`qsub -N $JOB_ID_2 -hold_jid $JOB_ID $align_script $SAMPLE_ID $OUTPUT_DIR $somaticPi_dir $SEQ_PLATFORM $Library_ID_prefix`;
}
