...
while (my $line = <VCF> ) {
	...
	if($line =~ m/^\#/) {
		...
		if ( $gene_name_ens_id ne "" ) {
			...
			$gene_name = $temp[0];
			$ens_id = $temp[1];
			#$genecard_link = "=HYPERLINK(\"http://www.genecards.org/cgi-bin/carddisp.pl?gene=$gene_name&search=$ens_id\", \"GeneCard Link\")";
		} else {
			$gene_name = "NA";
			$ens_id = "NA";
			#$genecard_link = "N/A";
		...
		$maf =~ s/^\,//;
		push @output, $elements[0]."\t".$elements[1]."\t".$elements[2]."\t".$elements[3]."\t".$elements[4]."\t".$elements[5]."\t".$elements[6]."\t".$elements[7]."\t".$SNPorINDEL ."\t".$elements[8]."\t".$Sample_Call_processed."\t".$gene_name."\t".$ens_id."\t".$isInterested."\t".$maf."\t".$omim_anno."\n";
		...
		if (scalar(@variants) > 1) {
			for (my $i=0;$i<scalar(@variants);$i++) {
				...
				push @output, $elements[0]."\t".$elements[1]."\t".$elements[2]."\t".$elements[3]."\t".$ALTs[$i]."\t".$elements[5]."\t".$elements[6]."\t".$elements[7]."\t".$V_type."\t".$elements[8]."\t".$Sample_Call_processed."\t".$gene_name."\t".$ens_id."\t".$isInterested."\t".$single_maf."\t".$omim_anno."\n";
			}
		}
	}
}
