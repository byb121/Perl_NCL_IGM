...
my %Variants_on_samples; # to record all rare variants in a new manner [Gene][sample]=chr_v-position_genoType;chr_v-position_genoType;
		...
		if($rr_na_sum < scalar @sample_names) { # RR and NA are removed
			...
			if( $elements[6] <= 0.05 && $elements[7] <= 0.05 ) {
				...
				for( my $i=0; $i< scalar @sample_columns;$i++) {
					...
					unless (exists $all_gene_id{$elements[$length-8]}) { $all_gene_id{$elements[$length-8]} = $elements[$length-9]; }
					if ( exists $Variants_on_samples{$sample_names[$i]}{$elements[$length-8]} ) {
						$Variants_on_samples{$sample_names[$i]}{$elements[$length-8]} = $Variants_on_samples{$sample_names[$i]}{$elements[$length-8]}.";".$elements[27]."_".$elements[28]."_".$elements[$sample_col+1];
					} else {
						$Variants_on_samples{$sample_names[$i]}{$elements[$length-8]} = $elements[27]."_".$elements[28]."_".$elements[$sample_col+1];
					}
				}
					...
					start_loop: foreach my $start_gene ( sort {$a<=>$b} keys %{$en_genes{$chr}} ) {
						foreach my $end_gene (keys %{$en_genes{$chr}{$start_gene}}) {
							if ( $start_gene <= $end_CNV && $end_gene >= $start_CNV ) {
								...
								my $gene_name = $temp[0];
								my $gene_id = $temp[1];
								unless (exists $all_gene_id{$gene_id}) { $all_gene_id{$gene_id} = $gene_name; }
...
my @compound_header = ('Gene_ID', 'Gene_Name');
foreach my $sample (@sample_names) {
...
my $number_of_wide_cols = (scalar @sample_names)*2+1;
$worksheet_compound -> set_column( 0,$number_of_wide_cols , 19 );
$worksheet_compound -> set_column( 1,$number_of_wide_cols , 13 );
$worksheet_compound -> set_column( 2,$number_of_wide_cols , 25 );


my %gene_sample_CNV_count;
my %gene_sample_VAR_count;
...
foreach  my $sample (@sample_names) {
	foreach my $gene_id (keys %{$Variants_on_samples{$sample}}) {
		...
		foreach my $var_info (@temp) {
			if ($var_info !~ m/R\/R$/ && $var_info !~ m/NA$/) {
		...
		#testing here
		#print $Variants_on_samples{$sample}{$gene}." Count result ".$gene_sample_VAR_count{$sample}{$gene}."\n";
	}
}
...
foreach my $gene_id (keys %all_gene_id) {
	...
	my $output_light = "red";
	push  @output, $gene_id;
	push  @output, $all_gene_id{$gene_id};
	my $row_height =1;
	foreach my $sample (@sample_names) {
...
if (scalar @sample_names >= 2) {
	...
	$worksheet_compound_AR -> set_column( 0,$number_of_wide_cols , 19 );
	$worksheet_compound_AR -> set_column( 1,$number_of_wide_cols , 13 );
	$worksheet_compound_AR -> set_column( 2,$number_of_wide_cols , 25 );
	...
	foreach my $gene_id (keys %all_gene_id) {
		...
		foreach my $sample_sample (keys %gene_sample_count) {
			...
			#print $sample_sample.$gene_sample_count{$sample_sample}.$output_light."\t";
		}
		#print "\n";
